...
PHANTOM_PATHS = [
    "/phantom/",                    # Docker container root
    "/phantom/memory/",             # 3-tier vector memory store
    "/phantom/config/",             # evolving config (Apply phase writes here)
    "/phantom/tools/",              # dynamically registered MCP tools
    "/phantom/sessions/",           # session storage
    "/phantom/observations/",       # Observe phase output (T79 entry point)
    "~/.phantom/",                  # user-level config
    "~/.phantom/config.json",       # primary evolving config file
    "~/.phantom/tools/",            # registered tools
    "~/.phantom/memory/",           # vector memory
]
