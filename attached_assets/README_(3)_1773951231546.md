...
[![1582 tests](https://img.shields.io/badge/tests-1582_passing-000?style=flat-square&labelColor=000)](tests/)

| | | | | | | |
|---|---|---|---|---|---|---|
| **76** threat families | **4** agents | **18** inspection triggers | **20** campaign patterns | **Learns your deployment** | **Network intelligence** | **Zero dependencies** |
...
### OpenShell — any agent, zero config

When any agent runs inside an OpenShell sandbox, Aiglos auto-detects and configures:
...
session = openshell_detect(guard=guard)  # reads OPENSHELL_POLICY_PATH automatically
...
OpenShell enforces what the agent is allowed to do (sandbox, YAML policy).
Aiglos detects what the agent is actually doing (behavioral sequences, campaigns, signed artifacts).
Both operate outside the agent process. Neither can be reasoned around.
...
**v0.25.1 — March 2026** · 1,582 tests

OpenShell agent-agnostic integration. `aiglos/integrations/openShell.py` — `is_inside_openShell()` detects the OpenShell sandbox from env vars. `openShell_context()` returns full sandbox context. `openshell_detect(guard)` auto-configures from `OPENSHELL_SANDBOX_ID` / `OPENSHELL_POLICY_PATH` / `OPENSHELL_AGENT`. `attach_openShell()` explicit attach for any agent. `attach_for_claude_code()`, `attach_for_codex()`, `attach_for_cursor()`, `attach_for_openclaw()` agent-specific helpers. Same YAML policy enforces identically for all agents — the sandbox is agent-agnostic by design. 9 known agents: Claude Code, Codex, Cursor, OpenClaw, Windsurf, Aider, Continue, Cody.

**v0.25.0 — March 2026** · 1,582 tests

NemoClaw integration. `aiglos/integrations/nemoclaw.py` — reads NemoClaw OpenShell YAML policy, extracts allowed hosts/tools/filesystem scope, registers with T69/T76 enforcement engine. `NeMoClawPolicy`, `NeMoClawSession`, `mark_as_nemoclaw_session()`. T76 `NEMOCLAW_POLICY_BYPASS` (score 0.95, critical) — fires when agent writes to `.nemoclaw/`, `.jensenclaw/`, or `openShell.yaml` at runtime. `NEMOCLAW_POLICY_HIJACK` campaign pattern (confidence 0.96). `sandbox_type="openShell"` added to recognized sandbox types. `OpenClawGuard.attach_nemoclaw()`. 76 total threat families. 20 campaign patterns. Announced: GTC 2026, March 17.

**v0.24.0 — March 2026** · 1,514 tests · T71-T75 from ATLAS review. T75 SESSION_DATA_EXTRACT (ATLAS T-DISC-002). atlas_coverage.py: 22 threats, 19 full, 3 partial, 0 gaps, 93%. GHSA watcher automation. aiglos scan-exposed. GOVBENCH.
