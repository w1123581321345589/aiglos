...
---------------------
Setting policy="federal" enables:
  - CMMC Level 2 event logging
  - §1513-compatible attestation artifact on every heartbeat cycle
  - Sub-agent chain attestation (Doraemon -> Ada -> Prism hierarchy)
  - Immutable signed log written to ./aiglos_federal.log
...
class SessionArtifact:
    ...
    @property
    def ndaa_1513_ready(self) -> bool:
        return self.policy == "federal"
    ...
    def summary(self) -> str:
        ...
        ndaa = "COMPLIANT" if self.policy == "federal" else "N/A (use policy='federal')"
        lines.append(f"  NDAA §1513  : {ndaa}")
        return "\n".join(lines)
...
class OpenClawGuard:
    ...
    def before_tool_call(
        ...
        try:
            ...
            pass  # ByteRover check is non-fatal
...
def attach(
    ...
    Subsequent calls to aiglos_openclaw.check() use this guard.
        ...
        import aiglos_openclaw
        aiglos_openclaw.attach(policy="federal")
    """
