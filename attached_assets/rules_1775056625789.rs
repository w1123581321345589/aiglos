...
fn match_t86_cross_tenant(_name: &str, args: &str) -> bool {
    // Simplified: fires on explicit source/dest tenant mismatch
    let s = args.to_lowercase();
    s.contains("source_tenant_id") && s.contains("dest_tenant_id")
}

fn match_t87_threshold_probe(_name: &str, args: &str) -> bool {
    // T87 requires session-level state; stub for now
    // Full implementation tracks blocked call fingerprints across the session
    let _ = args;
    false
}
...
pub static RULES: &[ThreatRule] = &[
    ...
    // T02-T80 stubs -- full match functions being ported from Python
    // See https://github.com/aiglos/aiglos for the complete Python taxonomy
];
