...
Answers a publicly documented "specific Tuesday" question:
  "Do you know what your system did last Tuesday -- not what it was
   configured to do, not what it was supposed to do, but what it
   actually did?"
...
Usage:
  ...
  aiglos forensics query --policy federal
...
All output is to stdout. Exit codes:
  0 -- success (records found or stats printed)
  1 -- no records found
...
def cmd_query(args: argparse.Namespace) -> int:
    """Query the forensic store and print matching sessions."""
    store = ForensicStore()
...
def cmd_stats(args: argparse.Namespace) -> int:
    """Print summary stats about the forensic store."""
    store = ForensicStore()
    stats = store.stats()
