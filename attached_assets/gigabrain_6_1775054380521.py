...
def declare_hermes_supervisor(
    ...
    pipeline_name:   str  = "hermes-supervisor",
) -> dict:
    """
    Register a Hermes supervisor relationship with the Aiglos guard.

    Declares the inter-agent coordination channel as a known surface,
    suppresses T28 false positives on legitimate [STATUS_REQUEST] /
    [REVIEW_REQUEST] exchanges, and activates T83 enforcement for
    ACK suppression, status spoofing, and protocol violations.

    peer_mention:   Discord mention token for the Hermes bot (<@BOT_ID>).
                    Required for T83 spoofing detection — messages claiming
                    to be STATUS_REQUEST without this token are suspicious.
    channel_id:     The #operator-ai channel ID. Registers with T83's
                    _T83_REGISTERED_CHANNELS so protocol violations fire.
    intent_markers: Custom marker set. Defaults to the standard four:
                    [STATUS_REQUEST], [REVIEW_REQUEST], [ESCALATION_NOTICE], [ACK]
    pipeline_name:  Identifier for logging.

    Returns a session dict with registration details.
    ...
    Example:
        ...
        session = declare_hermes_supervisor(
            guard,
            peer_mention = "<@1478282749755392026>",
            channel_id   = "1234567890123456789",
        )
        # T83 now active on #operator-ai
        # T28 suppressed for legitimate Hermes <-> OpenClaw exchanges
        # Hermes declared as known sub-agent (T23 suppressed)
    """
    if intent_markers is None:
        intent_markers = [
            "STATUS_REQUEST", "REVIEW_REQUEST",
            "ESCALATION_NOTICE", "ACK",
        ]

    # Register the coordination channel with T83 enforcement engine
    if channel_id:
        try:
            from aiglos.core.threat_engine_v2 import _T83_REGISTERED_CHANNELS
            _T83_REGISTERED_CHANNELS.add(channel_id.lower())
            log.info("[Hermes] Registered coordination channel %s with T83", channel_id)
        except ImportError:
            log.debug("[Hermes] T83 not available — channel registration skipped")

    # Declare Hermes as a known sub-agent to suppress T23 false positives
    hermes_tools = [
        "messages_read", "messages_send", "messages_send",
        "channel.read", "channel.write", "discord.send",
    ]
    try:
        guard.declare_subagent(
            "hermes-supervisor",
            tools     = hermes_tools,
            hard_bans = ["deploy", "push_to_main", "fabricate_data"],
        )
        log.info("[Hermes] Declared hermes-supervisor as known sub-agent")
    except Exception as e:
        log.debug("[Hermes] Sub-agent declaration error: %s", e)
    ...
    session = {
        ...
        "t83_active":     bool(channel_id),
        "t23_suppressed": True,
        "note": (
            "Hermes/OpenClaw case study: Hermes rebuilt the research pipeline "
            "autonomously. Same behavior with adversarial intent = T82 + T83."
        ),
    }

    if not hasattr(guard, "_hermes_sessions"):
        guard._hermes_sessions = []
    guard._hermes_sessions.append(session)

    log.info(
        "[Hermes] Supervisor registered pipeline=%s channel=%s T83=%s",
        pipeline_name, channel_id or "unset", session["t83_active"],
    )
    return session
