...
SAMPLE_POLICY_DICT = {
    ...
    "filesystem": {
        ...
        "deny_write": ["/home/user/.ssh/"],
    },
    "tools": {
        "allowed": ["filesystem", "web_search"],
        "denied": ["sessions.list"],
    },
    "privacy": {"strip_pii": True, "local_only": False},
}
...
class TestOpenShellDetect:
    ...
    def test_returns_none_without_policy_path(self, tmp_path):
        _set_openShell_env()  # no policy path
        g = _guard(tmp_path)
        result = openshell_detect(guard=g)
        assert result is None
...
class TestAttachOpenShell:
    ...
    def test_missing_policy_graceful(self, tmp_path):
        ...
        assert result is None
        # Guard should still have sandbox context set
        assert g.sandbox_context is True
...
class TestAgentSpecificHelpers:
    ...
    def test_all_agents_same_policy_same_enforcement(self, tmp_path):
        ...
        This is the architecture insight: guardrails outside the agent don't know
        what agent is inside them.
        """
        p = _policy_file(tmp_path)
        sessions = []
        ...
        # All sessions should have the same policy hash
        hashes = {s.policy.policy_hash for s in sessions if s}
        assert len(hashes) == 1, \
            f"Same policy file produced different hashes: {hashes}"

    def test_t76_fires_for_any_agent(self, tmp_path):
        """T76 fires regardless of which agent is inside OpenShell."""
        p = _policy_file(tmp_path)
        from aiglos.core.threat_engine_v2 import match_T76
        # Policy bypass is agent-agnostic
        assert match_T76("filesystem.write_file",
                         {"path": ".nemoclaw/policy.yaml"}) is True


# =============================================================================
# KNOWN_AGENTS completeness
# =============================================================================
