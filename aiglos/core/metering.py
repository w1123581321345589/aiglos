...
@dataclass
class UsageEvent:
    event_type: str         # "tool_call" | "block_event" | "session_end"
    tool_name: str
    verdict: str
    risk_type: str
    api_key: str | None
...
class MeterClient:
    ...
    # ── Sync path (used from sync wrappers) ───────────────────────────────────
    ...
    def record(self, tool_name: str, args: dict, result: ScanResult,
        ...
        with self._lock:
            ...
            if cfg.is_free_tier and self._call_count > cfg.free_limit:
                ...
                return   # don't buffer -- no key, no telemetry

            if not cfg.api_key:
                return   # free tier, local-only
    ...
    # ── Async path (used from async wrappers, fire-and-forget) ────────────────

    async def record_async(self, tool_name: str, args: dict, result: ScanResult,
                           config: AiglosConfig | None = None) -> None:
        # Delegate to sync under lock (fast), then flush if batch full
    ...
    def _post_batch(self, batch: list[UsageEvent]):
        ...
        # 3-second timeout -- never blocks the agent
        try:
